...
function buildStops(days: DayData[]): Stop[] {
  const out: Stop[] = []
  let idx = 0
  for (const d of days) {
    for (const leg of d.legs) {
      if (leg.latitude && leg.longitude) {
        out.push({ dayNumber: d.dayNumber, leg, dayDesc: (d.description as Record<string, string>)?.en || "", flatIndex: idx })
        idx++
      }
    }
  }
...
export function ItineraryDetailClient({ itinerary }: { itinerary: ItineraryData }) {
  ...
  const stops = buildStops(itinerary.days)
  ...
  const mapPoints: StoryPoint[] = stops.map((s, i) => ({
    lat: s.leg.latitude!,
    lng: s.leg.longitude!,
    label: (s.leg.name as Record<string, string>)?.en || `Stop ${i + 1}`,
  ...
  // ── Trap wheel events inside timeline card ─────────────────────────
  useEffect(() => {
    ...
    const handler = (e: WheelEvent) => {
      ...
      // Only let main scroll take over if at the very top/bottom
      if (!atTop && !atBottom) {
        e.stopPropagation()
      }
    }
    ...
    try {
      ...
      await fetch("/api/enquiries", {
        ...
        body: JSON.stringify({
          ...
          yachtName: name.en || "Itinerary Enquiry",
          type: "enquiry",
          checkIn: today,
          checkOut: today,
  ...
  const legName = (stop.leg.name as Record<string, string>)?.en || ""
  const legDesc = (stop.leg.description as Record<string, string>)?.en || ""
  ...
  return (
    <>
          ...
          <div ref={headerRef} className="absolute bottom-0 left-0 right-0 z-[3] pointer-events-none">
            <div className="max-w-[1400px] mx-auto px-6 lg:px-12 pb-6 md:pb-8 pointer-events-auto">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-semibold tracking-[0.1em] uppercase text-[#58D6F1]/70">Sailing Itinerary</span>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-white/60 text-sm">
                {itinerary.startFrom && <span className="flex items-center gap-1.5"><Anchor className="size-3.5" /> From {itinerary.startFrom}</span>}
                {itinerary.totalDays > 0 && <span className="flex items-center gap-1.5"><Navigation className="size-3.5" /> {itinerary.totalDays} Days</span>}
          ...
          {stops.length > 0 && (
            <div className="absolute bottom-36 left-0 right-0 md:hidden z-10 px-4">
              <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-none">
                {stops.map((s, i) => {
                  const sName = (s.leg.name as Record<string, string>)?.en || `${i + 1}`
      ...
      {/* ═══════════════════════════════════════════════════════════════
          FIXED OVERLAY CARDS — stay in place while map is in viewport
          ═══════════════════════════════════════════════════════════════ */}
            ...
            {Array.from(dayGroups.entries()).map(([dayNum, dayStops]) => (
              <div key={dayNum} className="mb-0.5">
                <div className="flex items-center gap-2 mb-0.5 py-1.5">
                  ...
                  <span className="text-[9px] font-bold tracking-[0.1em] uppercase" style={{ color: "#0055a9" }}>Day {dayNum}</span>
                  <div className="h-px flex-1 bg-black/[0.06]" />
                </div>
                <div className="relative ml-[10px] border-l border-black/[0.08] pl-3.5">
                  {dayStops.map((s) => {
                    const sName = (s.leg.name as Record<string, string>)?.en || `Stop ${s.flatIndex + 1}`
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {isActive && hasImages && (
                            <span className="text-[8px] font-medium mt-0.5 block" style={{ color: "#0055a9" }}>{(s.leg.images as string[]).length} photo{(s.leg.images as string[]).length > 1 ? "s" : ""}</span>
                          )}
                        </div>
      ...
      {stops.length > 0 && (
        ...
        >
          <div className="p-4 lg:p-5">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[9px] font-bold tracking-[0.1em] uppercase" style={{ color: "rgba(88,214,241,0.7)" }}>Day {stop.dayNumber}</span>
              <div className="size-1 rounded-full bg-white/15" />
              <span className="text-[9px] text-white/30 tabular-nums">{active + 1} of {stops.length}</span>
            </div>
            <h3 className="text-lg lg:text-xl font-bold mb-2" style={{ fontFamily: "var(--font-display)", letterSpacing: "-0.01em", color: "#ffffff" }}>{legName || `Stop ${active + 1}`}</h3>
              ...
              <div className="group/gallery cursor-pointer" onClick={() => setLightbox({ images: legImgs, index: 0 })}>
                <div className="flex items-center gap-2 mb-2.5">
                  <span className="px-2 py-0.5 rounded-full text-[9px] font-bold" style={{ background: "linear-gradient(135deg, rgba(0,85,169,0.3), rgba(88,214,241,0.15))", color: "#58D6F1" }}>
                    {legImgs.length} Photo{legImgs.length > 1 ? "s" : ""}
                  </span>
                </div>
      ...
      <section className="relative overflow-hidden" style={{ background: "#000A1E" }}>
        ...
        <div className="relative max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          <div className="flex items-center justify-between py-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-mono tracking-wider" style={{ color: "rgba(88,214,241,0.4)" }}>
                ORIGIN {itinerary.startLatitude && itinerary.startLongitude ? `${Math.abs(itinerary.startLatitude).toFixed(4)}°${itinerary.startLatitude >= 0 ? "N" : "S"} ${Math.abs(itinerary.startLongitude).toFixed(4)}°${itinerary.startLongitude >= 0 ? "E" : "W"}` : itinerary.startFrom || "—"}
              </span>
            </div>
            ...
            <div className="flex items-center gap-2">
              <span className="text-[9px] font-mono tracking-wider" style={{ color: "rgba(88,214,241,0.4)" }}>
                {stops.length > 0 && stops[stops.length - 1].leg.latitude ? `${Math.abs(stops[stops.length - 1].leg.latitude!).toFixed(4)}°N ${Math.abs(stops[stops.length - 1].leg.longitude!).toFixed(4)}°E` : "DEST"}
              </span>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 pb-8">
            {[
              { icon: Calendar, label: "Duration", value: `${itinerary.totalDays}`, unit: "Days", color: "#58D6F1" },
              { icon: Route, label: "Distance", value: `${itinerary.totalMiles}`, unit: "NM", color: "#0077B6" },
              { icon: Anchor, label: "Departure", value: itinerary.startFrom || "—", unit: "", color: "#0055a9" },
              { icon: MapPin, label: "Stops", value: `${stops.length}`, unit: "Locations", color: "#58D6F1" },
            ].map((stat, i) => (
              <div
                key={i}
            ...
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
              ...
              <div>
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <span className="text-[11px] font-bold tracking-[0.12em] uppercase" style={{ color: "#0055a9" }}>The Voyage</span>
                ...
                <h2 className="text-3xl md:text-4xl lg:text-[2.75rem] font-bold leading-[1.15] mb-8" style={{ fontFamily: "var(--font-display)", letterSpacing: "-0.02em", color: "#0D1B2A" }}>
                  {name.en || "Untitled Route"}
                </h2>
                <p className="text-base md:text-[17px] leading-[1.9] mb-8" style={{ color: "#3D4551", maxWidth: "32em" }}>{shortDesc.en}</p>
                ...
                <div className="flex flex-wrap items-center gap-2.5 mb-8">
                  {itinerary.startFrom && (
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full" style={{ background: "rgba(0,33,71,0.04)", border: "1px solid rgba(0,33,71,0.06)" }}>
                      ...
                      <span className="text-xs font-medium" style={{ color: "#0D1B2A" }}>From {itinerary.startFrom}</span>
                    </div>
                  )}
              ...
              <div className="relative">
                {itinerary.defaultMedia ? (
                  <div className="relative rounded-2xl overflow-hidden cursor-pointer group" style={{ boxShadow: "0 24px 64px rgba(0,10,30,0.15)" }} onClick={() => setLightbox({ images: [itinerary.defaultMedia!], index: 0 })}>
                    <div className="aspect-[4/5]">
                      <img src={itinerary.defaultMedia} alt={name.en || ""} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-[1.03]" />
                    </div>
                    <div className="absolute inset-0 bg-gradient-to-t from-[#0D1B2A]/70 via-transparent to-transparent" />
                    <div className="absolute bottom-0 left-0 right-0 p-6">
                      <span className="text-[10px] font-bold tracking-[0.1em] uppercase" style={{ color: "rgba(255,255,255,0.75)" }}>{itinerary.totalDays}-Day Voyage</span>
                      <p className="text-lg font-bold text-white mt-1" style={{ fontFamily: "var(--font-display)" }}>{itinerary.startFrom || "Mediterranean"}</p>
                    </div>
                ...
                <div className="absolute -bottom-4 -left-4 md:-left-6 px-5 py-3 rounded-xl" style={{ background: "linear-gradient(135deg, #0055a9, #002147)", boxShadow: "0 8px 32px rgba(0,10,30,0.25)" }}>
                  ...
                  <span className="text-[10px] font-semibold tracking-[0.06em] uppercase block" style={{ color: "rgba(255,255,255,0.7)" }}>Stops</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <section>
        {itinerary.days.map((day, dayIdx) => {
          const dayDesc = (day.description as Record<string, string>)?.en || ""
              ...
              <div className="max-w-[1400px] mx-auto">
                <div className={`grid lg:grid-cols-2 min-h-[400px] ${dayIdx % 2 === 1 ? "direction-rtl" : ""}`}>
                  ...
                  {heroImg ? (
                    ...
                    >
                      <img src={heroImg} alt={`Day ${day.dayNumber}`} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-[1.03]" />
                    ...
                    <div className={`relative flex items-center justify-center ${dayIdx % 2 === 1 ? "lg:order-2" : ""}`} style={{ minHeight: 360, background: isDark ? "rgba(255,255,255,0.02)" : "rgba(0,33,71,0.02)" }}>
                      <div className="text-center">
                        ...
                        <span className="text-[11px] font-bold tracking-[0.1em] uppercase" style={{ color: isDark ? "#58D6F1" : "#0055a9" }}>Day {day.dayNumber}</span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className={`flex flex-col justify-center px-8 md:px-12 lg:px-16 py-12 lg:py-16 ${dayIdx % 2 === 1 ? "lg:order-1" : ""}`}>
                    <div className="flex items-center gap-3 mb-6">
                      <span className="text-[11px] font-bold tracking-[0.12em] uppercase" style={{ color: isDark ? "#58D6F1" : "#0055a9" }}>Day {day.dayNumber}</span>
                    ...
                    <div className="flex flex-col gap-6">
                      {allLegs.map((leg, legIdx) => {
                        const lName = (leg.name as Record<string, string>)?.en || ""
                        const lDesc = (leg.description as Record<string, string>)?.en || ""
                            ...
                            {lImgs.length > 0 && (
                              <div className="group/gallery flex items-center gap-3 mt-2 cursor-pointer" onClick={() => setLightbox({ images: lImgs, index: 0 })}>
                                ...
                                <span className="text-[10px] font-medium ml-1" style={{ color: isDark ? "rgba(255,255,255,0.4)" : "rgba(0,33,71,0.4)" }}>
                                  {lImgs.length} photo{lImgs.length > 1 ? "s" : ""}
                                </span>
                              </div>
                            )}
                          </div>
        ...
        <section className="relative py-24 md:py-32 overflow-hidden" style={{ background: "#001229" }}>
          ...
          <div className="relative max-w-[1400px] mx-auto px-6 lg:px-12">
            <div className="text-center mb-14">
              <span className="text-[11px] font-bold tracking-[0.12em] uppercase" style={{ color: "#58D6F1" }}>Ports of Call</span>
      ...
      <section className="relative py-28 md:py-36 overflow-hidden" style={{ background: "#002147" }}>
        ...
        <div className="relative max-w-[1400px] mx-auto px-6 lg:px-12 text-center">
          ...
          <p className="text-base md:text-lg mb-12 max-w-2xl mx-auto leading-relaxed" style={{ color: "rgba(255,255,255,0.55)" }}>
            Every voyage is unique. Let our team tailor this itinerary to your preferences, or explore our curated fleet of charter yachts.
          </p>
                ...
                <div className="relative px-8 pt-10 pb-8 text-center" style={{ background: "linear-gradient(135deg, #070c26 0%, #0055a9 60%, #0077cc 100%)" }}>
                  ...
                  <div className="relative">
                    ...
                    <p className="text-white/70 text-sm leading-relaxed max-w-sm mx-auto">
                      Your enquiry for the <span className="text-white font-semibold">{name.en || "charter itinerary"}</span> has been received.
                    </p>
                  </div>
                </div>
                ...
                <div className="px-8 -mt-4 relative z-10">
                  <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-5">
                    <div className="flex items-center gap-3 mb-4 pb-4 border-b border-gray-100">
                      ...
                      <div className="flex-1">
                        <p className="text-xs font-semibold text-gray-800">IYC Charter Team</p>
                        <p className="text-[10px] text-gray-400">Charter Advisor</p>
                      </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-gray-400">Routes curated for {enquiryGuestCount} guest{enquiryGuestCount !== 1 ? "s" : ""}</p>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <>
                <div className="relative px-6 pt-6 pb-4" style={{ background: "linear-gradient(135deg, #070c26 0%, #0055a9 100%)" }}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-base font-bold text-white">Plan Your Itinerary Charter</h2>
                      <p className="text-[11px] text-white/60 mt-0.5">{name.en || "Custom sailing itinerary"}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="px-6 py-5">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <textarea value={enquiryForm.notes} onChange={(e) => setEnquiryForm({ ...enquiryForm, notes: e.target.value })} placeholder="Celebrations, dietary needs, preferred destinations..." rows={3} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-xs font-medium focus:outline-none focus:border-[#0055a9] focus:ring-1 focus:ring-[#0055a9]/20 transition resize-none bg-gray-50/50" style={{ color: "#070c26" }} />
                    </div>
                  </div>
                  ...
                  <p className="text-center text-[10px] text-gray-400 mt-2.5">No commitment required &middot; Response within 24h</p>
                </div>
              </>
            )}
          </div>
