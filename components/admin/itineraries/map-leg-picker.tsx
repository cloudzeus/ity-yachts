...
interface Props {
  ...
  onConfirm: (result: MapLegResult) => void
  title?: string
  description?: string
  confirmLabel?: string
...
export function MapLegPicker({ open, onOpenChange, onConfirm, title, description, confirmLabel, mapsKey }: Props) {
        ...
        <div className="px-5 pt-5 pb-3">
          <DialogHeader>
            ...
            <DialogDescription style={{ color: "var(--on-surface-variant)" }}>
              {description || "Click anywhere on the map to place a pin. We'll suggest a name based on the location."}
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        <div className="relative w-full" style={{ height: 420 }}>
          ...
          {pin && (
            ...
            >
              ...
              <button onClick={() => { setPin(null); setName(""); setSuggestedName("") }}>
        ...
        <div className="px-5 pb-5 pt-3 flex flex-col gap-3" style={{ borderTop: "1px solid var(--outline-variant)" }}>
          {pin ? (
            <>
              {reversing ? (
                ...
                <>
                  {suggestedName && (
                    <div className="flex items-center gap-2 text-xs" style={{ color: "var(--on-surface-variant)" }}>
                      <Navigation className="size-3" style={{ color: "var(--secondary)" }} />
                      Suggested: <strong style={{ color: "var(--on-surface)" }}>{suggestedName}</strong>
                    </div>
                  )}
                  <div className="flex flex-col gap-1">
                    <Label className="text-[10px] uppercase tracking-wide" style={{ color: "var(--on-surface-variant)" }}>Name (EN) *</Label>
                    <Input
                      value={name}
            ...
            <p className="text-xs text-center py-2" style={{ color: "var(--on-surface-variant)" }}>
              Click on the map to place a pin and set the leg location.
            </p>
          )}
        </div>
      </DialogContent>
