      ...
      <div className="relative z-10 w-full max-w-[1400px] mx-auto px-6 py-24 md:py-32">
        ...
        <div className="flex flex-col mb-16 relative">
          ...
          <div className="max-w-3xl">
            ...
            <h2 className="text-4xl md:text-5xl lg:text-6xl mb-5 tracking-wide" style={{ fontFamily: "var(--font-display)", color: "var(--text-heading)", perspective: "600px" }}>
              <ScrollTypewriter as="span">
                <span className="inline-block font-light">Elevate</span>{" "}
                <span className="inline-block font-light">Your</span>{" "}
                <span className="inline-block font-extrabold" style={{ color: "var(--iyc-ionian-600)" }}>Voyage</span>
              </ScrollTypewriter>
            </h2>

            <ScrollTypewriter as="p" delay={0.35} className="svc-subtitle text-sm md:text-base leading-relaxed max-w-[620px]" style={{ fontFamily: "var(--font-body)", color: "var(--text-muted)" }}>
          ...
          >
            ...
            <div className="flex gap-4">
              <div className="text-center">
                ...
                <span className="text-[10px] uppercase tracking-wider block" style={{ color: "rgba(255,255,255,0.82)" }}>{tUpper("home.services.voyages", "Voyages")}</span>
              </div>
              <div className="w-px" style={{ background: "rgba(255,255,255,0.25)" }} />
              <div className="text-center">
                <span className="text-2xl font-light text-white">18+</span>
                <span className="text-[10px] uppercase tracking-wider block" style={{ color: "rgba(255,255,255,0.82)" }}>{tUpper("home.services.yachts", "Yachts")}</span>
              </div>
            </div>
          </div>
        ...
        <div className="absolute inset-0 p-8 flex flex-col justify-end">
          <div className="mb-auto flex justify-end">
            <div className="w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300" style={{ background: "rgba(132,119,110,0.1)" }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#0F79BE" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
              </svg>
            </div>
          </div>
          <div>
            {labelText && (
              <span className="text-xs font-semibold tracking-wider uppercase mb-3 block" style={{ color: "var(--text-on-inverse-muted)" }}>
                {labelText}
              </span>
            )}
            <h3 className={`${titleSize} mb-3 uppercase tracking-wide font-light`} style={{ color: "#05111F", fontFamily: "var(--font-display)", letterSpacing: "0.05em" }}>
              {title}
            </h3>
            {desc && (
        ...
        {horizontal ? (
          <>
            <div className="flex-1 md:pr-12">
              {labelText && (
                <span className="text-xs font-medium tracking-wider uppercase mb-3 block" style={{ color: "var(--text-subtle)" }}>
                  {labelText}
                </span>
              )}
              <h3 className={`${titleSize} mb-3 uppercase tracking-wide font-light`} style={{ fontFamily: "var(--font-display)", letterSpacing: "0.05em", color: "var(--text-heading)" }}>
                {title}
              </h3>
              {desc && (
                <p className="text-sm font-light max-w-lg leading-relaxed" style={{ color: "var(--text-muted)" }}>
                  {desc}
                </p>
              )}
            </div>
            <div className="mt-6 md:mt-0 flex-shrink-0">
              <div
                className="w-14 h-14 rounded-full flex items-center justify-center transition-all duration-300 group-hover:bg-[var(--iyc-taupe-500)] group-hover:text-[var(--text-heading)]"
                style={{ background: "rgba(255,255,255,0.10)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.28)", color: "#FFFFFF" }}
              >
          ...
          <>
            <div className="mb-auto flex justify-end">
              <div
                className="w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 group-hover:bg-[var(--iyc-taupe-500)] group-hover:text-[var(--text-heading)]"
                style={{ background: "rgba(255,255,255,0.10)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.28)", color: "#FFFFFF" }}
              >
