  ...
  return (
    <div className="w-full flex flex-col antialiased bg-white relative" style={{ color: "#070c26" }}>
            ...
            <div className="bg-white rounded-xl p-5 shadow-[0_4px_20px_rgb(0,0,0,0.06)] border border-gray-100">
              <div className="flex flex-col mb-4 pb-4 border-b border-gray-100">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="bg-green-50 text-green-700 border border-green-200 px-2.5 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wider flex items-center gap-1">
              ...
              <div className="flex flex-col gap-3">
                <div className="grid grid-cols-2 gap-2">
                  <div className="border border-gray-300 rounded-lg p-2.5 hover:border-gray-400 transition cursor-pointer">
                    <label className="block text-[9px] uppercase font-bold text-gray-500 mb-0.5">Check-in</label>
                    <div className="text-xs font-medium" style={{ color: "#070c26" }}>Add date</div>
                  </div>
                  <div className="border border-gray-300 rounded-lg p-2.5 hover:border-gray-400 transition cursor-pointer">
                    <label className="block text-[9px] uppercase font-bold text-gray-500 mb-0.5">Check-out</label>
                    <div className="text-xs font-medium" style={{ color: "#070c26" }}>Add date</div>
                  </div>
                </div>
                ...
                <button className="w-full text-white py-3 rounded-lg text-xs font-bold hover:opacity-90 transition duration-300 mt-1" style={{ backgroundColor: "#0055a9" }}>
                  Get an Offer
                </button>
      ...
      {weeklyPrices.length > 0 && (
        <section className="relative w-full py-24 px-6 md:px-10 mt-8" style={{ backgroundColor: "#070c26" }}>
          <div
            className="absolute -bottom-10 left-10 text-[180px] font-black whitespace-nowrap leading-none select-none tracking-tighter pointer-events-none uppercase"
            style={{ color: "rgba(255,255,255,0.02)" }}
          >
            Season Weekly Rate
          </div>

          <div className="max-w-[1400px] mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-8 items-start text-white">
            {/* Left label */}
            <div className="lg:col-span-2 flex flex-col items-start pt-2">
              <span className="text-white text-[10px] font-semibold px-3 py-1.5 rounded mb-4" style={{ backgroundColor: "#84776e" }}>
                Season {new Date().getFullYear()}
              </span>
              <h2 className="text-white text-base font-normal mb-0.5">{yacht.name}</h2>
              <h3 className="text-white text-2xl font-bold mb-6 tracking-tight">Weekly Rates</h3>
              <Link
                href="#"
                className="border border-white/30 text-white hover:bg-white hover:text-[#070c26] transition duration-300 px-5 py-2 rounded-lg text-xs font-medium"
              >
                Show Details
              </Link>
            </div>
            ...
            {/* Price cards */}
            <div className="lg:col-span-6 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {weeklyPrices.map((price, i) => {
                const from = new Date(price.dateFrom)
                const to = new Date(price.dateTo)
                return (
                  <div
                    key={i}
                    className="border border-white/20 rounded-xl p-3 flex flex-col hover:border-white/40 transition cursor-default"
                  >
                    <span className="text-[10px] font-semibold uppercase tracking-wider mb-1" style={{ color: "#84776e" }}>
                      {MONTH_NAMES[from.getMonth()]}
                    </span>
                    <span className="text-white/60 text-[10px] mb-1">
                      {formatDate(price.dateFrom)} - {formatDate(price.dateTo)}
                    </span>
                    <span className="text-white text-base font-semibold mb-2 tracking-tight">
                      {formatPrice(price.price, price.currency)}
                    </span>
                    <div className="rounded px-1.5 py-0.5 inline-flex self-start border" style={{ backgroundColor: "rgba(0,85,169,0.3)", borderColor: "rgba(0,85,169,0.5)" }}>
                      <span className="text-[8px] text-white/80 uppercase tracking-widest font-medium">
                        Per Week + VAT + APA
                      </span>
                    </div>
                  </div>
            ...
            <div className="lg:col-span-3 flex justify-end items-start pt-2">
              <div className="rounded-xl p-5 w-full max-w-[280px] shadow-2xl relative overflow-hidden" style={{ backgroundColor: "#8C7D70" }}>
                <div className="absolute -right-10 -top-10 w-40 h-40 bg-white/5 rounded-full blur-2xl" />
                <p className="text-white text-sm leading-snug font-medium mb-5 relative z-10 pr-2">
                  Would you like to receive a quote for this yacht?
                </p>
                <div className="mt-4 relative z-10">
                  <button className="w-full bg-white py-2.5 rounded-lg text-xs font-bold hover:bg-gray-100 transition shadow-sm" style={{ color: "#070c26" }}>
                    Enquire
                  </button>
                </div>
              </div>
